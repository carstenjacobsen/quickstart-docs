...
Public Key: GBZXN7PIRZGNMHGA7MUUUF4GWPY5AYPV6LY4UV2GL6VJGIQRXFDNMADI Secret Key: SC5O7VZUXDJ6JBDSZ74DSERXL7W3Y5LTOAMRF7RQRL3TAGAPS7LUVG3L 
...
## Testnet
The testnet network mode is recommended for software development. Accounts created on the testnet can be funded with Friendbot (faucet) so you don’t have to worry about losing real tokens when testing your software. The Stellar Lab can connect to Quickstart, which makes it easy to create and fund accounts.
...
CAUTION
The local network in this container is not suitable for any production use as it has a fixed root account. Any private network intended for production use would also require a unique network passphrase.
